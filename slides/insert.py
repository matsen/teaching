...
def text_to_path(img_name):
    ...
    if os.path.exists(nofonts_name) and os.path.getmtime(img_name) < os.path.getmtime(
        nofonts_name
    ):
        return nofonts_name
    # else...
    subprocess.run(
        "inkscape --export-text-to-path --export-filename={} {}".format(
...
def replace_fonts(func):
    ...
    def inner(img_name, *args, **kwargs):
        if img_name.endswith(".svg") and "_refontified" not in img_name and "_nofonts" not in img_name:
...
def note(text):
    return E.E("aside", text, **{"class": "notes"})
...
def get_fun(line):
    if line.startswith("^"):
        return tostring(map_fun(shlex.split(line[1:]))).decode("utf-8") + "\n"
    if line.startswith("(*"):
        return ""
    return line
